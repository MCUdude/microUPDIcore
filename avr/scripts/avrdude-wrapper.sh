...
if "${avrdudePath}" "-C$configPath" -v "-p$buildMCU" "-c$uploadProtocol" "-P$serialPort" "-b$uploadSpeed" -D "-Ueeprom:w:$runtimePlatformPath/firmwares/mEDBG_UPDI.eep:i" "-Uflash:w:$runtimePlatformPath/firmwares/mEDBG_UPDI.hex:i" 2>&1 | tee /dev/stderr | grep --silent "verification error, first mismatch at byte 0x7000"; then
