...
"%avrdudePath%" "-C%configPath%" -v -p%buildMCU% -c%uploadProtocol% "-P%serialPort%" -b%uploadSpeed% -D "-Ueeprom:w:%runtimePlatformPath%/firmwares/mEDBG_UPDI.eep:i" "-Uflash:w:%runtimePlatformPath%/firmwares/mEDBG_UPDI.hex:i"> "%buildPath%/avrdudeoutput.txt" 2>&1
